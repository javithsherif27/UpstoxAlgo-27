    ...
    <div className="flex h-screen relative">
      ...
      <aside className="w-56 bg-gray-900 text-gray-100 flex flex-col">
        ...
        <nav className="flex-1 space-y-1 px-2">
          ...
          <NavLink to="/app/orders" className={({isActive})=>`block px-3 py-2 rounded text-sm ${isActive?'bg-gray-700':'hover:bg-gray-800'}`}>Orders</NavLink>
      ...
      <main className="flex-1 overflow-auto bg-gray-50">
        {/* Global top header with Streaming Status */}
        <div className="sticky top-0 z-10 bg-white border-b border-gray-200">
          <div className="px-4 py-2 flex items-center justify-between">
            <div className="text-sm font-medium text-gray-700">Live Market Stream</div>
            <StreamingStatus />
          </div>
        </div>
        <Outlet />
